...
from .mmlfd_override import DrawOptions as MMDrawOptions
...
class PushTEnv(gym.Env):
    ...
    * TODO:
    ...
    def _initialize_observation_space(self):
        ...
        elif self.obs_type == "pixels_environment_state_agent_pos":
            self.observation_space = spaces.Dict(
                {
                    "environment_state": spaces.Box(
                        low=np.zeros(16),
    ...
    def step(self, action):
        ...
        self.cur_step += 1
        if not terminated and self.cur_step > 300:
            truncated = True
        else:
            truncated = False
...
class MMPushTEnv(gym.Env):
    ...
    def step(self, action, dummy_reward=False):
        ...
        if self.compute_reward() == 1.0 or (self._t + 1) >= self.max_episode_length:
        ...
        # done = reward == 1.0 or (self._t + 1) >= self.max_episode_length
        terminated = is_success = reward > 0.95
        if not terminated and (self._t + 1) >= self.max_episode_length:
            truncated = True
        else:
            truncated = False
        # done = reward > 0.95 or (self._t + 1) >= self.max_episode_length
        observation = self.get_obs()
        info = self._get_info()
        info["is_success"] = is_success
        info["coverage"] = reward
        self._t += 1

        return observation, reward if not dummy_reward else 0.0, terminated, truncated, info
    ...
    def _render_frame(self, mode, visualize):
        ...
        draw_options = MMDrawOptions(canvas)
